...
public class SaleController {
    ...
    public ResponseEntity<ResponseDTO> getSales() {
        return new ResponseEntity<>(
                new ResponseDTO(200, saleService.getSales(), null), HttpStatus.OK);
    }
    ...
    public ResponseEntity<ResponseDTO> getBestSeller() {
        ...
        if (saleService.getTotalSales() == 0) {
            return new ResponseEntity<>(new ResponseDTO(409, "No hay vendedores. ", null), HttpStatus.BAD_REQUEST);
        } else {
    ...
    @GetMapping(path = "/totalventastiendas/{code}")
    public ResponseEntity<ResponseDTO> getTotalventastiendas(@PathVariable String code) {
    ...
    public ResponseEntity<ResponseDTO> getBestStore() {
        ...
        if (storeService.getStores().size()==0){
            return new ResponseEntity<>(new ResponseDTO(409, "No hay tiendas. ", null), HttpStatus.BAD_REQUEST);
        } else {
